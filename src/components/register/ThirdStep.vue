  ...
  <div class="third">
    <section>
      <div class="content">
        <h3>用户须知</h3>
        <p>
          本站刊载的所有内容，包括文字、图片、音频、视频、软件、程序、以及网页版式设计等均在网上搜集。
        </p>
        <p>
          注册时要填写以下项目：用户名-中英文即可；密码-不易过于简单。建议6位以上，数字、字母、字符，至少2种混合；Email-必填，作为登录账号；性别-请选择男或女；
        </p>
      </div>
      <b-collapse
        :open="false"
...
export default {
  ...
  methods: {
    submit() {
      ...
      const name = this.$store.state.registeruser.name;
      const userSex = this.$store.state.registeruser.userSex;
      const userAge = this.$store.state.registeruser.userAge;
      const userSchool = this.$store.state.registeruser.userSchool;
      const type = 1;
